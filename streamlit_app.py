...
def pick_display_from_props(props: dict, node_type: str | None = None) -> str | None:
    ...
    3) 마지막 fallback: props 첫 번째 값
       (주의: Neo4j properties 맵의 순서는 완전 보장되지 않을 수 있음)
    """
    if not props:
        return None
...
st.caption("검색하면 생성된 Cypher/결과/서브그래프를 보여주고, 노드 라벨은 props 기반으로 자동 결정합니다.")
